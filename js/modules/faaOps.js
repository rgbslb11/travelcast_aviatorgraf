...
export function renderFaaOps() {
  ...
  document.querySelector("#faa-ops").innerHTML = `
    <div class="card">
    ...
    <div class="card">
      ...
      <div class="table-shell"><table>
        <thead><tr><th>Airport</th><th>Type</th><th>Status</th><th>Reason</th><th>Rate</th><th>Avg Delay</th><th>Max Delay</th><th>Scope</th><th>Expiration</th></tr></thead>
        <tbody>${d.ground_programs.map(gdpRow).join("")}</tbody>
      </table></div>
    </div>
    <div class="card">
      <h3>Planned Initiatives</h3>
      ${d.planned_initiatives.map(initiativeCard).join("")}
    </div>`;
}

function gdpRow(g) {
  const statusClass = g.status === "Active" ? "red" : "amber";
  return `<tr>
    ...
    <td><span class="badge ${statusClass}">${safeText(g.status)}</span></td>
...
function initiativeCard(i) {
  const statusClass = i.status === "Active" ? "amber" : "gray";
  return `<div style="border-top:1px solid var(--line);margin-top:12px;padding-top:12px">
    <span class="badge ${statusClass}">${safeText(i.status)}</span>
